...
db.users.updateOne({married: true}, {$set : {married : false}})

// updateMany({filter}, {data})

// User yang umurnya dibawah 30
db.users.updateMany({age :{$lt : 30}}, {$set : {address: {default: 'Jakarta', secondary: 'Bandung'}}})

// User yang umurnya mulai dari 30 ke atas
db.users.updateMany({age : {$gte: 30}}, {$set : {pets: ['Cat', 'Bird', 'Dog']}})


// DELETE
db.users.find({})

db.users.deleteOne({married: false})

db.users.deleteOne({married: false, age: {$gte: 20, $lte: 27}})

db.users.deleteMany({married: true})
